...
import { 
  X, Tag, LayoutGrid, FileText, AlertCircle, ShieldCheck, Loader2
} from 'lucide-react';
import { postServiceType } from "../../store/CrudServicio";
import Swal from "sweetalert2";

interface CreateTipoProps {
  onClose: () => void;
  onUpdated: () => void; // Para refrescar la tabla en el componente padre
}

export default function CreateModalTipo({ onClose, onUpdated }: CreateTipoProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Toast.fire({
        icon: 'success',
        title: 'Tipo de servicio creado exitosamente',
        customClass: {
        ...
        <div className="relative px-6 py-5 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-semibold text-slate-800 tracking-tight">Nuevo Tipo</h2>
                <p className="text-xs text-slate-500 font-medium">Registra una nueva categoría de servicio</p>
              </div>
            </div>
            <button 
              onClick={onClose} 
